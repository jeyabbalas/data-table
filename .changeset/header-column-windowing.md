...
- **A filter or derived-column edit panel closes when the column it is anchored to scrolls out of the window.** It has nowhere left to point, and following the scroll to whatever column took that position would be worse than closing. If you had focus inside the panel, it lands on `.dt-grid` rather than on the trigger button that is being removed with the header — restoring it there would end keyboard navigation on `<body>` and drag the horizontal scroll back toward the column you just scrolled away from.
...
- **The grid chunk grew 76.95 → 79.52 kB brotli, and the root entry 10.86 → 11.13 kB.** That is the window arithmetic, the keyed reconcile and the incremental header refreshes, and every table pays it whether or not it is wide enough to benefit. Stated rather than absorbed, as the previous release did for its own 2.7 kB.
...
- `ModalHost.close()`, `FilterPanel.close()` and `DerivedColumnEditPanel.close()` take an optional `{ restoreFocus?: boolean }` that overrides the choice made at `open()` time. Pass `{ restoreFocus: false }` when you are about to detach the element that opened the modal and will park focus yourself; the default is unchanged. On `@jeyabbalas/data-table/advanced`.
