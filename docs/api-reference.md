...
| `QueryOptions`        | interface | Per-query options for `WorkerBridge.query` — `{ cache?: boolean; priority?: 'high' \| 'normal' \| 'low' }`. `cache: false` bypasses both the read and the write of the SQL result cache. The worker's serial dispatch queue drains strictly `'high'` → `'normal'` (default) → `'low'`: `'high'` is for viewport row fetches, `'low'` for header charts, column-stats scans, and any full-table scan the user is not waiting on. |
...
| Symbol                              | Kind           | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TableContainer`                    | class          | Main DOM container that composes every UI piece. `announce(message)` speaks a transient message through its second polite live region. `refreshColumnWindow()` recomputes the header's and the body's column windows together and re-renders whichever moved; `getColumnHeaders()` returns the headers **currently mounted**, which off screen is not one per column.                                                                                                |
...
| `TableBody`                         | class          | Row rendering inside the scroller, on both axes: rows are windowed by `VirtualScroller`, columns by the body itself. `getColumnWindow()` reports which columns the rendered rows were built for; `refreshColumnWindow()` recomputes it synchronously (call it after writing `scrollLeft` yourself — the browser does not dispatch `scroll` until the current task ends); `getColumnSpan(column)` and `getPinnedWidthPx()` give the horizontal geometry to scroll to. |
| `ColumnWindow`                      | interface      | What `getColumnWindow()` returns: `pinnedCount` force-rendered leading columns, the `[start, end)` window into `visibleColumns`, both spacer widths, the pinned band width, and the total content width. Cells **and headers** outside it are **not in the DOM** — scroll a column into view before selecting either by `data-column`.                                                                                                                               |
...
| `StatsPanelCoordinatorOptions`      | interface      | Trailing ctor argument: `{ vizScheduler? }`. A scheduler governs filter _changes_ only — `syncExistingFilters` always broadcasts to every panel, because the facade's load gate awaits it.                                                                                                                                                                                                                                                                           |
...
| `TableContainer.refreshColumnWindow()`       | method   | Recompute the header and body column windows and re-render whichever moved. Synchronous; call after writing `scrollLeft` yourself. |
...
| `NumericHistogramSnapshot`, `DateHistogramSnapshot`, `TimeHistogramSnapshot`, `IntervalHistogramSnapshot`, `ValueCountsSnapshot`                                                   | types                  | What each class's `exportDataSnapshot()` returns. Park one across a header rebuild and pass it back as `VisualizationOptions.initialSnapshot` to rehydrate the replacement instance with zero DuckDB queries — valid only on another instance of the same class for the same column. |
| `CrossfilterCoordinator`                                                                                                                                                           | class                  | Broadcasts filter changes to registered visualizations.                                                                                                                                                                                                                              |
| `FilterFanOutScheduler`, `FilterFanOutRequest`                                                                                                                                     | types                  | Optional seam both coordinators take as `options.vizScheduler`: `refreshOnFilters(request)` replaces the per-registration loop, so an implementation picks which of `request.columns` to `request.refresh()` now and which to defer.                                                 |
...
Source: `src/DataTable.ts`. Validates options, initializes a `WorkerBridge`, builds `TableState`/`StateActions`, mounts the UI into `options.container`, wires events/persistence/presets/undo-redo, and resolves once the UI is mounted. If `options.source` is provided, the initial load is awaited, so the returned table is already populated — but only to **first interactive paint**: rows painted and filter counts correct, column charts possibly still fetching. Await [`whenVizReady()`](#methods) or subscribe to `vizReady` when you need the charts drawn; a load failure rejects the returned promise and emits `loadError`.

---

## `CreateDataTableOptions`

Source: `src/DataTable.ts:123-325`.
...
| `visualizations`        | `boolean \| { eager?: boolean }`               | `true`                         | Auto-attach column-header histograms and value counts. `true` / `{}` is lazy — a column's chart is created and fetched when its header scrolls into view, and the load promise does not wait for it. `false` disables charts entirely. `{ eager: true }` creates and fetches every applicable column's chart during load and holds the load promise until they settle (screenshot/PDF pipelines). See [Visualizations → When charts are created](./guides/visualizations.md#when-charts-are-created). |
...
Returned by `createDataTable()`. Source: `src/DataTable.ts:330-419`.
...
| --------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `loadData`            | `(source, opts?) => Promise<void>`                            | Load a new source. Emits `loadStart` → `loadProgress` → `loadComplete` (or `loadError`). Resolves at first interactive paint — rows painted, filter counts correct — not when the column charts have drawn.                                       |
| `whenVizReady`        | `() => Promise<void>`                                         | Resolves when the current load's visible column charts have finished fetching — the promise form of `vizReady`. Replaced on each `loadData`; already resolved by the time `loadData` resolves under `visualizations: false` or `{ eager: true }`. |
...
| `setOnFilterRemove`        | `(cb: (column: string) => void) => void` | Called once per column that loses its filter — by chip, panel, `clearFilters`, a preset, undo/redo, reset, or a retyped derived column. Not called when a filter is merely replaced. Use it to clear state keyed to a filter, such as a chart's brush. |
...
| `loadProgress`    | `ProgressInfo`                                                                                                       | Load progress, `percent` 0–100 and non-decreasing, ending in exactly one `100`. Stages in emission order: `reading` (main thread, `loaded`/`total` in bytes, the only `cancelable: true` stage), then `parsing`, `analyzing` (`loaded`/`total` = type-probe chunks), `indexing` from the worker.                 |
| `loadComplete`    | `{ tableName, rowCount, schema }`                                                                                    | First interactive paint: data loaded, schema known, first rows painted, filter counts correct. Does **not** mean the column charts are drawn.                                                                                                                                                                    |
| `vizReady`        | `{ tableName: string; vizCount: number }`                                                                            | Once per load, when the column charts visible at load time have finished fetching. `vizCount` is the wave's size (visible plus overscan), `0` when nothing was visible or `visualizations: false`. After `loadComplete` by default; before it under `{ eager: true }` or `false`. Not emitted if the load fails. |
...
| `LOAD_PARSE_FAILED`       | `LoadError`             | `src/worker/dispatcher.ts`                                                         | CSV/JSON/Parquet parse failed (fallback code for any uncoded loader error).                                                                          |
