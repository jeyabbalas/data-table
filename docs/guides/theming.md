...
table.on('warning', ({ code }) => {
  if (code === 'STYLESHEET_MISSING') {
    console.warn(
      'Import @jeyabbalas/data-table/styles (or the CSS file) to render the UI properly.',
    );
  }
});
```
...
- **Dark mode has two trigger systems.** `prefers-color-scheme: dark` _and_ `[data-dt-color-scheme="dark"]`. The attribute wins when explicitly set. To follow OS again, call `setColorScheme('auto')`.
