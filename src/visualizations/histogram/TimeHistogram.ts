 ...
 * - Human-readable time labels (12-hour format)
 * - Responsive sizing
 *
 * Similar to DateHistogram but specialized for TIME type (seconds from midnight).
 */
...
export class TimeHistogram extends BaseVisualization {
  ...
  async fetchData(): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[TimeHistogram] Failed to fetch data for ${this.column.name}:`,
        error
      );
      this.data = null;
      this.render();
