  ...
  for (const [vectorType, expectedDuckDBType] of Object.entries(typeToExpected)) {
    it(`${vectorType} → ${expectedDuckDBType}`, async () => {
      ...
      await actions.addDerivedColumn({
        kind: 'vector',
        name: 'col',
        vectorType: vectorType as VectorDataType,
        values: dummyValues,
      });
...
function getDummyValues(type: VectorDataType, count: number): any[] {
  switch (type) {
    ...
    case 'decimal':
      return Array.from({ length: count }, (_, i) => (i * 0.123).toFixed(6));
    case 'string':
      return Array.from({ length: count }, (_, i) => `val_${i}`);
    case 'boolean':
    ...
    case 'date':
      return Array.from({ length: count }, (_, i) => `2024-01-${String(i + 1).padStart(2, '0')}`);
    case 'timestamp':
      return Array.from(
        { length: count },
        (_, i) => `2024-01-${String(i + 1).padStart(2, '0')} 10:30:00`,
      );
    case 'time':
      return Array.from({ length: count }, (_, i) => `${String(i).padStart(2, '0')}:30:00`);
    case 'interval':
      return Array.from({ length: count }, (_, i) => `${i + 1} day`);
  }
}
