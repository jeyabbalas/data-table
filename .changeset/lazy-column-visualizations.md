...
- **Chart data outlives the chart's DOM.** Reclaiming a canvas snapshots its data; scrolling the column back rebuilds the chart from the snapshot with no query. Hiding, showing, pinning, or moving a column issues no visualization queries at all.
...
- **`visualizations` accepts `boolean | { eager?: boolean }`.** `true` / `undefined` / `{}` is the lazy default; `false` is unchanged; `{ eager: true }` turns the visibility gate off, so a column's chart is created and fetched the moment its header exists rather than when the header nears the viewport, and the load promise waits for all of them. Because the header row is windowed in this same release, "every header that exists" is the mounted window rather than every column — see the header windowing entry.
