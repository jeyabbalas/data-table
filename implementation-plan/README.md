...
| 4 — Large-scale browser e2e suite            | [phase-4-browser-e2e.md](./phase-4-browser-e2e.md)                     | [x] done — bigTable helper + extent/flicker specs green at 1.6M in Chromium (mid-storm probe: zero violations; negative control confirmed vs the 33.55M px clamp); 50M-row bench holds the 1 ms/16.6 ms budgets; RUN_DUCKDB_PERF rowid-walk ~150 ms. Deviations: exact-cap guard asserts spacer content height (scrollHeight gets a one-row band); scrollToRow(last) asserted bottom-flush; sorted-mode determinism runs at 200K |
| 5 — Docs, changeset, manual verification     | [phase-5-docs-and-verification.md](./phase-5-docs-and-verification.md) | [x] done — all docs + api-reference aligned to the shipped architecture (typedoc tree regenerated); house-prose minor changeset; live-Chrome verification against the 1.5M-row bug parquet passed every step; full PR gate green                                                                                                                                                                                                 |
